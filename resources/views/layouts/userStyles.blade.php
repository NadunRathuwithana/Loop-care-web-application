{{-- Bootstrap --}}
<link href="{{ url('https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css') }}" rel="stylesheet">

{{-- font --}}
<link href="{{ url('https://fonts.cdnfonts.com/css/sf-pro-display') }}" rel="stylesheet">

{{-- font awesome --}}
<link rel="stylesheet" href="{{ url('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.3.0/css/all.min.css') }}"
    integrity="sha512-SzlrxWUlpfuzQ+pcUCosxcglQRNAq/DZjVsC0lE40xsADsfeQoEypE+enwcOiGjk/bSuGGKHEyjSoQ1zVisanQ=="
    crossorigin="anonymous" referrerpolicy="no-referrer" />

{{--  Swiper JS  --}}
<link rel="stylesheet" href="{{ url('https://cdn.jsdelivr.net/npm/swiper@10/swiper-bundle.min.css') }}" />

{{-- custom css --}}
<link href="{{ url('assets/css/style.css') }}" rel="stylesheet">
<link href="{{ url('assets/css/nav.css') }}" rel="stylesheet">
<link href="{{ url('assets/css/player.css') }}" rel="stylesheet">
